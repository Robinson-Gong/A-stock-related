# -*- coding: utf-8 -*-
"""
Created on Wed Jan 20 15:40:13 2021
...
def get_official_data():
    ...
    time.sleep(10)
    driver.get("http://www.cnindex.com.cn/sample-detail/download-history?indexcode=399102")
    time.sleep(3)
    driver.close()
...
class ratio():
    ...
    def deal_data(self):
        ...
        df = read_data(r'399102_cons.xlsx')
        ...
        df = df.dropna()
        save_data(df, r'399102_cons.xlsx')
        return df
        ...
        for j in range(len(trade_date)):
            if len(time_series) - 1 != 0:
                ...
                for k in range(100):
                    ...
                    dataframe.loc[j * 100 + k, 'name'] = df.iloc[k, 2]
                    dataframe.loc[j * 100 + k, 'vol'] = df.loc[k, 'volume']
        save_data(dataframe,r'399102_detail.xlsx')
        con = sql_connect()
        print(r'获取日数据')
    ...
    def getresult(self):
        ...
        save_data(result,r'399102_权重.xlsx')
        CYBZ = pd.read_excel('399102_权重.xlsx')
...
print(r'获取官网数据')
if os.path.isfile(r'399102_cons.xlsx'):
    os.remove(r'399102_cons.xlsx')
get_official_data()
ratio.getresult()
